...
namespace Ruyi.SDK.Online
{
    ...
    public class RuyiNetVideoService : RuyiNetService
    {
            ...
            EnqueuePlatformTask(index, () =>
            {
                ...
                if (uploadedFile.status == 200)
                {
                    ...
                    if (cdnUrl.status == 200)
                    {
                        ...
                        mClient.BCService.Entity_CreateEntity(VIDEO_LOCATION, videoEntity, acl, index);
                    }
                }
        ...
        public void DownloadVideo(int index, string cloudFilename, RuyiNetTask<RuyiNetResponse>.CallbackType callback)
        {
            EnqueuePlatformTask(index, () =>
            {
                return mClient.BCService.File_DownloadFile(VIDEO_LOCATION, cloudFilename, true, index);
            }, callback);
        }
        ...
        public void ListVideos(int index, RuyiNetTask<RuyiNetListUserFilesResponse>.CallbackType callback)
        {
            EnqueueTask(() =>
            {
                return mClient.BCService.File_ListUserFiles_SNSFO(VIDEO_LOCATION, false, index);
            }, callback);
        }
            ...
            RuyiNetTask<RuyiNetUploadFileResponse>.CallbackType callback)
        {
            EnqueueTask(() =>
            {
                return mClient.BCService.File_DeleteUserFile(VIDEO_LOCATION, cloudFilename, index);
            }, callback);
        }
            ...
            RuyiNetTask<RuyiNetGetCDNResponse>.CallbackType callback)
        {
            EnqueueTask(() =>
            {
                return mClient.BCService.File_GetCDNUrl(VIDEO_LOCATION, cloudFilename, index);
            }, callback);
        }
        /// <summary>
            ...
            RuyiNetTask<RuyiNetGetFriendsVideosResponse>.CallbackType callback)
        {
            EnqueueTask(() =>
            {
                ...
                return mClient.BCService.Entity_GetSharedEntitiesListForProfileId(profileId, query, "{}", 100, index);
            }, callback);
        }
